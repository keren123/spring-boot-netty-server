...
/* package */abstract class AbstractCommand<R> implements HystrixInvokableInfo<R>, HystrixObservable<R>
{
    ...
    {
        Observable<R> result = getFallbackOrThrowException(this, HystrixEventType.TIMEOUT, FailureType.TIMEOUT, "timed-out", new TimeoutException());
        Reference<TimerListener> tl = timeoutTimer.get();
        if (tl.get().isTimeOut())
        {
...
//            executionResult = executionResult.markUserThreadCompletion((int) userThreadLatency);
            ...
            //todo 发送事件
//            metrics.markCommandDone(executionResult, commandKey, threadPoolKey, false);
